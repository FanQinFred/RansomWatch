...
{
	if (Globals::Instance->getCommCloseStat())
	{
		std::cerr << "No com to close" << std::endl;
		return;
	}
...
BOOLEAN AntiRansomWareApp::MyForm::openKernelDriverCom()
{
	if (context.Port == nullptr)
	{
		...
		try
		{
			resOpen = FilterConnectCommunicationPort(ComPortName, 0, nullptr, 0, nullptr, &context.Port);
		}
		catch (Exception ^ e)
		{
		...
		DBOUT("Success to open mini filter port to driver" << std::endl);
		Globals::Instance->setCommCloseStat(FALSE);
	}
	return TRUE;
}
